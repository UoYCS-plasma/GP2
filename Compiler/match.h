...
/* Deletes and frees the first element from the mapping. */        
...
/* Create a new assignment specified by the last two arguments and prepend it
 ...
 *
 * compareAtoms returns false if the expressions do not match:
 ...
 *
 * If the passed value differs from the value assigned to the variable in the
 * current assignment, the functions return false. If the values are equal,
 * the function returns true. If the variable does not exist in the assignment,
 * a copy of list/value is dynamically allocated and added to the assignment.
 *
 ...
 * Argument 3: The value required for a label match. This is a pointer to heap
 *             memory. If it is not added to assignment, it is freed inside
 *             the function. It is the responsibility of the caller to free the
 *             memory in the case that the function returns false.
 */                    

bool verifyListVariable(Assignment **assignment, string name, GList *list);
bool verifyAtomVariable(Assignment **assignment, string name, ListElement *value);
