...
static void emitRootNodeMatcher(Rule *rule, RuleNode *left_node, SearchOp *next_op)
{
   ...
   if(left_node->interface == NULL)
        PTFI("IF_INVALID_DANGLING_NODE(%d, %d, %d) continue;\n", 6,
             left_node->indegree, left_node->outdegree, left_node->bidegree);
   else PTFI("IF_INVALID_NODE(%d, %d, %d) continue;\n\n", 6,
             left_node->indegree, left_node->outdegree, left_node->bidegree); 
                                    ...
                                    SearchOp *next_op)
{
   ...
   /* The node is deleted by the rule if it has no corresponding node in the interface. */
   if(left_node->interface == NULL)
        PTFI("IF_INVALID_DANGLING_NODE(%d, %d, %d) %s\n", 3, left_node->indegree, 
             left_node->outdegree, left_node->bidegree, fail_code);
   else PTFI("IF_INVALID_NODE(%d, %d, %d) %s\n\n", 3, left_node->indegree,
             left_node->outdegree, left_node->bidegree, fail_code);
   ...
   if(type == 'b')
   {
      ...
      /* The node is deleted by the rule if it has no corresponding node in the interface. */
      if(left_node->interface == NULL)
         PTFI("IF_INVALID_DANGLING_NODE(%d, %d, %d) return false;\n", 6,
               left_node->indegree, left_node->outdegree, left_node->bidegree);
      else PTFI("IF_INVALID_NODE(%d, %d, %d) return false;\n\n", 6,
               left_node->indegree, left_node->outdegree, left_node->bidegree); 
      PTFI("}\n", 3);
   }
   ...
   for(index = 0; index < rule->lhs->edge_index; index++)
   {
      ...
      else
      {
         ...
         if(edge->interface->relabelled)
         {
            ...
            PTFI("bool relabel_edge%d = !equalHostLabels(label_e%d, label);\n", 3, index, index);
